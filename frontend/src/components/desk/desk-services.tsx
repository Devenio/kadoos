"use client";

import { deskFieldClass, useDesk } from "@/components/desk/desk-shell";
import { Button } from "@/components/ui/button";
import {
  deskCreateService,
  deskReorderServices,
  deskServices,
  deskUpdateService,
} from "@/lib/api/desk";
import { formatDuration, formatPrice, text } from "@/lib/shop-format";
import type { DeskService } from "@/types/desk";
import { useEffect, useState } from "react";

const empty = { nameEn: "", nameFa: "", durationMin: 30, priceToman: 0 };

export function DeskServices() {
  const { locale, dictionary, fail, toast } = useDesk();
  const copy = dictionary.desk;
  const [rows, setRows] = useState<DeskService[]>([]);
  const [editing, setEditing] = useState<string | "new" | null>(null);
  const [form, setForm] = useState(empty);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    void deskServices().then(setRows).catch(fail);
  }, [fail]);

  function startEdit(row?: DeskService) {
    if (!row) {
      setEditing("new");
      setForm(empty);
      return;
    }
    setEditing(row.id);
    setForm({
      nameEn: row.name.en,
      nameFa: row.name.fa,
      durationMin: row.durationMin,
      priceToman: row.priceToman,
    });
  }

  async function save() {
    setBusy(true);
    try {
      if (editing === "new") {
        const created = await deskCreateService(form);
        setRows((current) => [...current, created]);
      } else if (editing) {
        const updated = await deskUpdateService(editing, form);
        setRows((current) => current.map((row) => (row.id === updated.id ? updated : row)));
      }
      setEditing(null);
      toast(copy.saved);
    } catch {
      fail();
    } finally {
      setBusy(false);
    }
  }

  async function move(index: number, delta: number) {
    const next = [...rows];
    const swap = index + delta;
    if (swap < 0 || swap >= next.length) {
      return;
    }
    [next[index], next[swap]] = [next[swap]!, next[index]!];
    setRows(next);
    try {
      setRows(await deskReorderServices(next.map((row) => row.id)));
    } catch {
      fail();
      void deskServices().then(setRows);
    }
  }

  return (
    <section className="mx-auto w-full max-w-2xl">
      <div className="flex items-center justify-between gap-3">
        <h1 className="font-display text-3xl">{copy.nav.services}</h1>
        <Button size="touch" onClick={() => startEdit()}>
          {copy.add}
        </Button>
      </div>
      <ul className="mt-6 divide-y divide-border border-y border-border">
        {rows.map((row, index) => (
          <li key={row.id} className="py-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-lg font-medium">{text(locale, row.name)}</p>
                <p className="text-sm text-muted-foreground">
                  {formatDuration(locale, row.durationMin)} · {formatPrice(locale, row.priceToman)}
                </p>
                {row.active ? null : (
                  <p className="mt-1 text-sm text-muted-foreground">{copy.archived}</p>
                )}
              </div>
              <div className="flex flex-wrap justify-end gap-2">
                <Button size="sm" variant="outline" onClick={() => void move(index, -1)}>
                  {copy.reorderUp}
                </Button>
                <Button size="sm" variant="outline" onClick={() => void move(index, 1)}>
                  {copy.reorderDown}
                </Button>
                <Button size="sm" variant="outline" onClick={() => startEdit(row)}>
                  {copy.edit}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => {
                    void deskUpdateService(row.id, { active: !row.active })
                      .then((updated) => {
                        setRows((current) =>
                          current.map((item) => (item.id === updated.id ? updated : item)),
                        );
                      })
                      .catch(fail);
                  }}
                >
                  {row.active ? copy.archive : copy.restore}
                </Button>
              </div>
            </div>
          </li>
        ))}
      </ul>

      {editing ? (
        <form
          className="mt-8 space-y-4"
          onSubmit={(event) => {
            event.preventDefault();
            void save();
          }}
        >
          <p className="text-sm text-muted-foreground">{copy.serviceWarn}</p>
          <label className="block">
            {copy.nameEn}
            <input
              required
              dir="ltr"
              value={form.nameEn}
              onChange={(event) => setForm((current) => ({ ...current, nameEn: event.target.value }))}
              className={deskFieldClass()}
            />
          </label>
          <label className="block">
            {copy.nameFa}
            <input
              required
              value={form.nameFa}
              onChange={(event) => setForm((current) => ({ ...current, nameFa: event.target.value }))}
              className={deskFieldClass()}
            />
          </label>
          <label className="block">
            {copy.duration}
            <input
              required
              type="number"
              min={5}
              value={form.durationMin}
              onChange={(event) =>
                setForm((current) => ({ ...current, durationMin: Number(event.target.value) }))
              }
              className={deskFieldClass()}
            />
          </label>
          <label className="block">
            {copy.price}
            <input
              required
              type="number"
              min={0}
              value={form.priceToman}
              onChange={(event) =>
                setForm((current) => ({ ...current, priceToman: Number(event.target.value) }))
              }
              className={deskFieldClass()}
            />
          </label>
          <div className="grid grid-cols-2 gap-3">
            <Button size="touch" type="submit" loading={busy}>
              {copy.save}
            </Button>
            <Button size="touch" type="button" variant="outline" onClick={() => setEditing(null)}>
              {copy.cancel}
            </Button>
          </div>
        </form>
      ) : null}
    </section>
  );
}
